...
if ($verb == "GET") {
    $array = array();
    $sql = "SELECT * FROM angon_event ORDER BY angon_event_id DESC";
...
if ($verb == "POST") {
    ...
    $arrayImage = array();
    $angon_event_id = mysqli_real_escape_string($con, $angon_event_id);
    ...
    if ($resultImage) {
        ...
        unlink($config['IMAGE_UPLOAD_PATH'] . '/angon_event_image/' . $arrayImage["angon_event_image"]);
    } else {
        if (DEBUG) {
    ...
    $deleteSql = "DELETE FROM angon_event WHERE angon_event_id = $angon_event_id";
